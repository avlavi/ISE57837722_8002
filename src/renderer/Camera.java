...
public class Camera {
    ...
    public List<Ray> constructRays(int nX, int nY, int j, int i) {
        ...
        double rY = height / nY;//height of a pixel
        double rX = width / nX;//width of a pixel
        double sRY = rY / numberOfRays;//height of a segment
        double sRX = rX / numberOfRays;//width of a segment
     ...
     * @param nX  The number of pixels in the x-axis of the view plane grid.
     * @param nY  The number of pixels in the y-axis of the view plane grid.
     * @param row The index of the pixel in the x-axis of the grid.
     * @param col The index of the pixel in the y-axis of the grid.
            ...
            while (threadsCount-- > 0) // add appropriate number of threads
                threads.add(new Thread(() -> { // add a thread with its code
                    ...
                    while ((pixel = pixelManager.nextPixel()) != null) {
                        ...
                        if (numberOfRays == 1) castRay(nX, nY, pixel.col(), pixel.row());//multithreading
                        else if (!adaptive) castRays(nX, nY, pixel.col(), pixel.row());//multithreading + antialiasing
                        else imageWriter.writePixel(pixel.col(), pixel.row(), AdaptiveSuperSampling(imageWriter.getNx(), imageWriter.getNy(), pixel.col(), pixel.row(), numberOfRays));//multithreading + antialiasing +  adaptive
                    }
                }));
     ...
     * @param numOfRays The sqrt of the amount of rays sent
     * @return Pixel color
     */
    private Color AdaptiveSuperSampling(int nX, int nY, int j, int i, int numOfRays) {
        ...
        return rayTracer.AdaptiveSuperSamplingHelper(pIJ, rX, rY, PRx, PRy, this.location, this.vRight, this.vUp, null);
    }
